	...
	void* pUserData)
{
	...
	if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT)
	{
		...
		EVO_ERROR("Validation layer: {}", pCallbackData->pMessage);
	}
	else if (messageSeverity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT)
	{
		//printWColor("\nvalidation layer WARNING: " << pCallbackData->pMessage, WARNCOL);
		EVO_WARN("Validation layer: {}", pCallbackData->pMessage);
	}
	else
	{
		//printWColor("validation layer INFO: " << pCallbackData->pMessage, INFOCOL);
		EVO_TRACE("validation layer: {}", pCallbackData->pMessage);
	}
	...
	: window_{ window }
{
	...
	EVO_INFO("Starting Instance Creation");
	createInstance();
	EVO_INFO("Setting up debug messenger");
	setupDebugMessenger();
	EVO_INFO("Creating surface");
	createSurface();
	EVO_INFO("Picking physical device");
	pickPhysicalDevice();
	EVO_INFO("Creating logical device");
	createLogicalDevice();
	EVO_INFO("Creating command pool");
	createCommandPool();
}
...
{
	EVO_INFO("Checking validation layer support");
...
bool Device::checkValidationLayerSupport()
{
	...
	for (const char* layerName : validationLayers)
	{
		...
		if (!layerFound)
		{
			EVO_ERROR("Layer was not found during Validation Layer Support checking! Layer name is: {}", layerName);
			return false;
		}
	}
